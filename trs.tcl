...
proc regexp_cmd { redex contractum vars lvl } {
    ...
    forindex i $regex {
        ...
        if { [operator? $t] } {
            ...
            set vars [concat $vars [lindex $drill 1]]
...
proc rule { redex -> contractum } {
    set transformer [regexp_cmd $redex $contractum {} 1]
    set regex [lindex $transformer 0]
    set vars  [lindex $transformer 1]
...
proc interact { } {
    ...
    while { ! [string equal $input quit] } {
        ...
        if { [llength $term] } { puts "expression tree: $term" }
        if { [valid $term] }   { puts [simplify $term $::rules] }
        set input [prompt "> "]
    }
}
